...
- Add isOAuthTokenRevokedByGID_Transaction to OAuthSQLStorage.
...
- Adds `BulkImportProxyStoragePool` / `openBulkImportProxyStoragePool(int)` and savepoint-capable `BulkImportProxySQLStorage`; deprecates `createBulkImportProxyStorageInstance()`
- Adds `Storage.verifySchema(boolean strictMode)` and `SchemaMismatchException` for the core's startup schema check
