 ...
 *
 * <p>These events form a distinct class from {@link ActivityEventType activity events} (sign-ins,
 * refreshes): lifecycle events are mandatory and unthrottled (written in the mutation's transaction,
 * exempt from the activity-log verbosity flag which gates activity pings only), whereas activity events
 * are best-effort/throttleable.
 *
 * <p>The string {@link #getValue() value} is what lands in the {@code activity_log.event_type} column;
 * the structured JSON that accompanies each event lives in the core-side {@code LifecycleEventPayload}.
 * This class defines the vocabulary only — it has no emit sites and no payload logic.
 */
public enum LifecycleEventType {
     ...
     * last-active rollup (see {@link RollupEventTypes}): an interactive sign-up may feed the rollup, but an
